define(['services/services','jquery'],function(services,$){
	...
	var DB = function($q,$http){
		this.read = function(setName, selector){
			...
			$http.jsonp("http://192.168.1.120:3000/dataService/find?callback=JSON_CALLBACK",
                {params:params})
		...
		this.save = function(setName, document){
			var d = $q.defer();
			$http.jsonp("http://192.168.1.120:3000/dataService/save?callback=JSON_CALLBACK",
		...
		this.remove = function(setName, document){
			var d = $q.defer();
			$http.jsonp("http://192.168.1.120:3000/dataService/remove?callback=JSON_CALLBACK",
				{params:{setName:setName,document:document}}
			).success(
				function(data){
					d.resolve(data);
